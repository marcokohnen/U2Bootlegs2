  ...
  ngOnInit() {
    ...
    findConcertsByTitle(lookUpValue: string) {
        this.concertService.findConcertsByTitle(lookUpValue)
            .subscribe(
                (concerts : Concert[]) => {this.resultList = concerts;
                                                //console.log("resultList = " + JSON.stringify(concerts))
                    },
